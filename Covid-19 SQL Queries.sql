--Looking at the Covid 19 Death table to understand its impact in out surroundings
...
-- I'll be looking at the percentage of Total Deaths per Total Cases
--This shows the likelyhood of dying after contacting the virus
...
--This shows what percentage of the population that got Covid
...
-- Looking at countries with the Higest Covid Rate per Population
...
--Lookin at countries with highest Death Rate per Population
...
-- Looking at the Global Covid Infection Rate
...
-- Looking at the correlation between Covid-19 vaccinations and Covid-19 deaths
...
SUM(CAST(CV.new_vaccinations AS FLOAT)) OVER (PARTITION BY CD.Location ORDER BY CD.Location, CD.date) AS Incremental_vac_count_byLocation -- To have the summation of new vaccinations per location per day
